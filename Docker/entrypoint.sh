...
initialize() {
    ...
    cp ./gateway-ha-config.yml "${APP_CONFIG}"
...
setup_mysql_dev_schema()
{
    ...
    /usr/bin/mysql -u"${DB_USER}" -p"${DB_PASS}" -h "${DB_HOST}" --port="${DB_PORT}" -D prestogateway < ./gateway-ha-persistence.sql
}

initialize
check_mysql_connection
setup_mysql_dev_schema
java -jar ./gateway-ha-${APP_VERSION}-SNAPSHOT-jar-with-dependencies.jar server "${APP_CONFIG}"